package com.gateway.common.adapter.web.filter;
...
import com.gateway.common.framework.identity.Validator;
...
public class SessionValidateFilter implements Filter {
	...
	public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain) throws IOException, ServletException {
		...
		for (String unsecureUri : this.unsecureUris) {
			...
			//对druid监控放行 start
			if(requestUri.indexOf("/druid/")>=0){
				unsecure = true;
			}
			//对druid监控放行 start
			
			if(requestUri.indexOf("/dwz/")>=0){
				unsecure = true;
			}
		...
		//验证权限
		if(!validator.validatePermission()){
			validator.cancel();
			HttpServletResponse response = (HttpServletResponse) res;
			StringBuffer sb = request.getRequestURL();
			String query = request.getQueryString();
			...
			if (ServerInfo.isAjax(request) || request.getParameter("ajax") != null) {
				PrintWriter out = response.getWriter();
				out.println("{\"statusCode\":\"300\", \"message\":\"No permission! Please system administrator!\"}");
			} else {
				// request.setAttribute("statusCode", 301); // for iframeDone
				// callback
				response.sendRedirect(response.encodeRedirectURL(request.getContextPath() + this.loginUrl + java.net.URLEncoder.encode(backToUrl, "UTF-8")));
