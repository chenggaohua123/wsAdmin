...
//import com.gateway.fraud.model.LoginAccount;
...
public class RequestUtil extends BaseController
{
	...
	public static Criteria getCaCriteria(HttpServletRequest request){
		...
		Integer pageNum = getInteger(request, "pageNum");
        if (pageNum == null || pageNum <= 0) {// 判断页码是否为空
            pageNum = 1;
        }
        Integer numPerPage = getInteger(request, "numPerPage");
	 ...
	 * @return
	 */
	/*public static LoginAccount getLogAccount(HttpServletRequest request){
		Object obj  =(LoginAccount)request.getSession().getAttribute(Constants.SESSION_LOGIN_USER);
		if(null == obj){
			return null;
		}
		return (LoginAccount)obj;
	}*/
