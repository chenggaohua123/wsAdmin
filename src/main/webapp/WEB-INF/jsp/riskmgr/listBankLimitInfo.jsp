...
function bankLimit_selectAll(){
	if ($("#bankLimit_selectAll").attr("checked")) {  
    	$("input[name='bankLimit_ids']").each(function(){
...
function deleteBankLimitInfo(){
	if(confirm("是否删除！")){
		var str=""; 
		 $("input[name='bankLimit_ids']:checked").each(function(){ 
			str+="ids="+$(this).val()+"&"; 
		});
		 alert(str);
		if(!str){
			...
			var url="<%=path %>"+"/riskmgr/deleteBankLimitInfo?"+str;
			$.ajax({
				url:url,
				type:'post',
				dataType:'json',
...
<form id="pagerForm" method="post" action="<%=path %>/riskmgr/listBankLimitInfo">
	<input type="hidden" name="pageNum" value="${page.nowPage }" />
	<input type="hidden" name="numPerPage" value="${page.pageSize}" />
</form>
<div class="pageHeader">
	<form rel="pagerForm" id="branklimitform" onsubmit="return navTabSearch(this);" action="<%=path %>/riskmgr/listBankLimitInfo" method="post">
	<div class="searchBar">
    ...
    <div class="panelBar">
		...
		<li><a class="add" href="<%=path %>/riskmgr/goAddBankLimitInfo" target="dialog" width="550" height="400" mask="true"  rel="settleType" ><span>添加</span></a></li>
			<li><a class="edit" href="<%=path %>/riskmgr/goUpdateBankLimitInfo?id={rateId}" target="dialog" width="550" height="400" mask="true"  rel="settleType"  warn="请选择修改的数据"><span>修改</span></a></li>
			<li><a class="delete" href="javascript:deleteBankLimitInfo()" id="deleteBankLimitInfo"  width="1000" height="600" mask="true"  rel="settleType1" ><span>批量删除</span></a></li>
		</ul>
	</div>
		<table class="list" width="100%" targetType="navTab" layoutH="189" style="text-align: center;">
			<thead>
				<tr>
					<th><input type="checkbox" name="bankLimit_selectAll" id="bankLimit_selectAll" onclick="bankLimit_selectAll()"></th>
					<th>商户号</th>
					<th>终端号</th>
					<th>银行</th>
					<th>国家</th>
			...
			<tbody>
				<c:forEach items="${page.data}" var="info">
					<tr  target="rateId" rel="${info.id }"   >
						<td><input type="checkbox" name="bankLimit_ids" value="${info.id }" id="bankLimit_ids"></td>
