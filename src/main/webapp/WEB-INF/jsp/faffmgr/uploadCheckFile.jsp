...
function downloadTransModel(){
	location.href="<%=path %>/faffmgr/downloadFaffModel?type="+$('#c_exportType').val();
}
</script>
<body>
<div class="pageHeader" >
<select id="c_exportType" onchange="changeType()">
	<option value="2">上传：交易数据核对</option>
	<option value="3">上传：拒付数据核对</option>
	<option value="4">上传：退款数据核对</option>
	<option value="5">上传：返还保证金数据</option>
</select><button onclick="downloadTransModel()">下载&nbsp;批量上传&nbsp;模板</button>
<div id="c_div_2" style="display:none;">
	<font color="red">
		<b>上传注意事项:<br>
		1.上传文件为txt类型 <br>
		2.文件内容:流水号、银行结算币种、银行结算金额、银行手续费、银行到账日期<br>
...
<div id="c_div_3" style="display:none;">
	...
	<b>上传注意事项:<br>
	1.上传文件为txt类型 <br>
	2.文件内容:流水号、银行扣款币种、银行扣款金额、银行扣款日期<br>
	3.分别用英文逗号隔开<br>
	4.第一行为目录<br>
	5.一行内容为一条订单<br>
...
<div id="c_div_5" style="display:none;">
	<font color="red">
	<b>上传注意事项:<br>
	1.上传文件为txt类型 <br>
	2.文件内容:4、流水号、银行返还币种、银行返还保证金金额、银行返回到账日期<br>
	3.分别用英文逗号隔开<br>
	4.第一行为目录<br>
	5.一行内容为一条订单<br>
	6.第一行为类型，第二行为标题，第一、二行不允许删除<br>
    ...
    <script type="text/javascript">
      $('#drag-and-drop-zone').dmUploader({
        url: '<%=path%>/faffmgr/acceptCheckFile',
        dataType: 'json',
        allowedTypes: 'text/*',
        ...
        onUploadSuccess: function(id, data){
          ...
          openWindow("<%=path %>/faffmgr/goQueryAcceptCheckPage");
