       ...
       <div class="tabs" currentIndex="0" eventType="click" style="width:100%">
		...
		<div>
           	<p>
                ...
                <input name="merNo" type="text" size="30" value="${terInfo.merNo }" readonly="readonly" class="required"/>
            </p>
            <p>
                <label>终端号：</label>
            ...
            <p>
                <label>结算币种：</label>
                <c:if test="${empty terInfo.settleCurrency }">
                 <input name="settleCurrency" type="text" size="30"   class="required"/>
                </c:if>
                 <c:if test="${! empty terInfo.settleCurrency }">
                <input name="settleCurrency" type="text" size="30"  value="${terInfo.settleCurrency }"  readonly="readonly" class="required"/>
                </c:if>
            </p>
            <p>
                <label>账户名称：</label>
            ...
            <p>
                <label>银行账号：</label>
                <input name="accountNo" type="text" size="30" value="${terInfo.accountNo }"  class="required"/>
            </p>
             <p>
                <label>收款银行：</label>
           ...
           <%--  <p>
            ...
            <p>
                <label>高风险通：</label>
                	<select name="isRisk">
                		<option ${terInfo.isRisk==0?'selected':'' } value="0">maxmind</option>
                		<option ${terInfo.isRisk==1?'selected':'' } value="1">ThreatMetrix</option>
                		<option ${terInfo.isRisk==2?'selected':'' } value="2">Insights</option>
                		<option ${terInfo.isRisk==3?'selected':'' } value="3">不走</option>
                	</select>
			    </select>
            </p>
              <p>
                <label>产品类型：</label>
                <select class="combox" loadurl="<%=path %>/sysmgr/queryBaseDateInfo?tableName=TER_PRODUCT_TYPE" relValue="columnKey" selectedValue="${terInfo.productType }" relText="columnvalue"  name="productType">
			    </select>
            </p>
             <p>
                <label>备注：</label>
            ...
            <p>
                <label>商户地址：</label>
                <input name="merchantAddress"   type="text" value="${terInfo.merchantAddress}"  size="30" class=""/>
            </p>
            <p>
                <label>SWIFT：</label>
                <input name="swift"   type="text" value="${terInfo.swift}"  size="30" class=""/>
            </p>
            <p>
                <label>银行地址：</label>
                <input name="bankAddress"   type="text" value="${terInfo.bankAddress}"  size="30" class=""/>
            </p>
            <p>
                <label>商户渠道：</label>
                <select class="combox" loadurl="<%=path %>/sysmgr/queryBaseDateInfo?tableName=MERCHANT_CHANNEL" relValue="columnKey" selectedValue="${terInfo.merchantChannel}" relText="columnvalue"  name="merchantChannel">
			    </select>
            </p>
            <p>
                <label>银行行号：</label>
                <input name="bankNo"   type="text" value="${terInfo.bankNo}"  size="30" class=""/>
            </p>
         </div>
