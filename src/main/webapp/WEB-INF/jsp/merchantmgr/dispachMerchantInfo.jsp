...
<div class="pageHeader">
	...
	<div class="searchBar">
		...
		<ul  class="searchContent">
			...
			<li>
			   <label>OA业务单号：</label>
			   <input type="text" name="oaOrderNo" value="${param.oaOrderNo }"/>
			</li>
		</ul>
		<ul  class="searchContent">
		...
		<ul  class="searchContent">
			 <li>
			   <label>开户状态：</label>
			   <select class="combox" name="accountStatus">
			...
			<li>
			   <label>商户等级：</label>
			   <select class="combox" name="merLev">
				<option value="">所有</option>
			   </select>
			</li>
			<li>
		...
		<ul  class="searchContent">
			<li>
				<label>通道类型：</label>
				<select class="combox" name="merCurrencyType">
				<option value="">所有</option>
			   </select>
			</li>
			 <li   class="dateRange">
			  <label>行业类型</label>
			  <select class="combox" name="industry">
				<option value="">所有</option>
			   </select>
			</li>
			<label>直连状态：</label>
				<select class="combox" name="dirStatus">
					<option value="">所有</option>
				</select>
			</li>
		</ul>
...
<div class="pageContent">
	...
	<table class="list" width="100%" targetType="dialogTab" layoutH="215" style="text-align: center;">
		<thead>
			<tr>
				<td><input type="checkbox" group="merchantNo" class="checkboxCtrl"></td>
				<th>商户号</th>
				<th>商户名称</th> 
				<th>开户状态</th>
				<th>登录状态</th>
		...
		<tbody>
			<c:forEach items="${page.data }" var="merchant">
				<tr target="sid_role" rel="${merchant.id }"    align="center">
					<td><input name="merchantNo" id="checkbox${merchant.merNo }" value="${merchant.merNo }" type="checkbox" 
				     <c:forEach items="${ list}" var="r">
				       <c:if test="${r.agentNo==requestScope.agentInfo.agentNo }">
				          checked="true"
				       </c:if>
				     </c:forEach> >
				   </td>
