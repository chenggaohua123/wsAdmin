		...
		<tbody>
			<c:forEach items="${page.data }" var="terInfo">
				<tr target="sid_role" rel="${terInfo.id }" >
					<td><a  href="<%=path %>/merchantmgr/queryTerInfo?id=${terInfo.id }" target="dialog" width="850" height="350" mask="true" title="终端详细信息" rel="mer">${terInfo.merNo }</a></td>
					<td>
					<a class="btnSelect" href="javascript:$.bringBack({merNo:'${terInfo.merNo }'})" title="查找带回">选择</a>
					</td>
				</tr>
			</c:forEach>
		</tbody>
