...
function batchAcceptCheckUpdateById(){
	...
	if(!str){
		...
		var url="<%=path %>"+"/settlemgr/batchMoneyInfo";
		$.ajax({
			url:url,
			type:'post',
			dataType:'json',
...
<div class="pageContent" style="width:100%;height:100%;clear:#183152;">
	<div class="panelBar">
		<ul class="toolBar">
			<li><a class="edit" href="javaScript:batchAcceptCheckUpdateById()"><span style="color:green;">批量出款</span></a></li>
			<li class="line">line</li>
			<li><a class="edit" href="javaScript:showData()"><span style="color:green;">显示未匹配数据</span></a></li>
			<li><span style="color:red;">警告：支付点击批量出款，选中信息会全部变更为出款</span></li>
		</ul>
	</div>
		...
		<tbody>
			<c:forEach items="${list}" var="trans">
				<tr  target="transCheckID" rel="${trans.id }"   >
					...
					<td>${trans.id }</td>
					<td>${trans.accountType==0?"交易账户":"保证金账户" }</td>
					<td>${funcs:getStringColumnKey('CASHTYPE',trans.cashType,'未知状态')}</td>
					<td>${trans.currency }&nbsp;${trans.amount }</td>
					<td>${trans.moneyBy }</td>
					<td>${trans.moneyDateStr }</td>
					<td>${trans.moneyRemark }</td>
				</tr>
			</c:forEach>
		</tbody>
	</table>
