...
<body>
<form id="pagerForm" method="post" action="<%=path %>/bankMgr/getCurrencyListBrightBack">
	<input type="hidden" name="pageNum" value="${page.nowPage }" />
	<input type="hidden" name="numPerPage" value="${page.pageSize}" />
</form>
<div class="pageHeader">
	<form rel="pagerForm" onsubmit="return dialogSearch(this);" action="<%=path %>/bankMgr/getCurrencyListBrightBack" method="post">
	<div class="searchBar">
		...
		<tbody>
			<c:forEach items="${page.data }" var="bank">
				<tr target="bankId" rel="${bank.id }">
					...
					<td>${bank.remark }</td>
					<td>
					<a class="btnSelect" href="javascript:$.bringBack({currencyId:'${bank.id }', bankName:'${bank.bankName }', currencyName:'${bank.currencyName }'})" title="查找带回">选择</a>
				</td>
				</tr>
			</c:forEach>
		</tbody>
