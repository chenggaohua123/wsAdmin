...
<body>
<form id="pagerForm" method="post" action="<%=path %>/transmgr/getTransChangeInfoList">
	<input type="hidden" name="pageNum" value="${page.nowPage }" />
	<input type="hidden" name="numPerPage" value="${page.pageSize}" />
</form>
<div class="pageHeader" >
	<form rel="pagerForm" onsubmit="return navTabSearch(this);" action="<%=path %>/transmgr/getTransChangeInfoList" method="post">
	<div class="searchBar" >
		...
		<ul class="searchContent">
			...
			<select class="combox"  name="type">
			      ...
			      <option value="0" ${param.type=='0'?'selected':'' }>勾兑成功</option>
			      <option value="1"	${param.type=='1'?'selected':'' }>强制失败</option>
		    </select>
			</li>
		</ul> 
		<div class="subBar">
		...
		<tbody>
			<c:forEach items="${page.data }" var="trans">
				<tr target="tradeNo" rel="${trans.tradeNo }">
					...
					<td>${trans.type==0?'失败勾兑成功':'强制失败' }</td>
					<td>${trans.createBy }</td>
				</tr>
			</c:forEach>
		</tbody>
	</table>
