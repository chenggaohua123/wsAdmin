		...
		<div class="tabs" currentIndex="0" eventType="click" style="width: 100%">
			...
			<div class="tabsContent pageFormContent">
				<div>
					...
					<dl>
						<dt>结算金额</dt>
						<dd>${transInfo.merSettleCurrency}
							${transInfo.merSettleAmount}</dd>
					</dl>
					<dl>
						<dt>银行交易金额</dt>
					...
					<dl>
						...
						<c:if test="${transInfo.respCode=='0000'}">
							<font color="blue">${funcs:getStringColumnKey('RESP_INFO',transInfo.respCode,transInfo.respCode)}</font>
						</c:if>
						<c:if test="${transInfo.respCode=='01'}">
					...
					<dl>
						...
						<c:if test="${transInfo.transDishonor=='1'}">
					...
					<dl>
						<dt>退款状态</dt>
						<c:if test="${transInfo.transRefund=='0'}">
							<font color="red">未退款</font>
						</c:if>
						<c:if test="${transInfo.transRefund=='1'}">
					...
					<dl>
						<dt>冻结状态</dt>
						<c:if test="${transInfo.transFrozen=='0'}">
					...
					<dl>
						<dt>冻结金额</dt>
						<dd>${transInfo.merBusCurrency}${transInfo.frozenAmount}</dd>
					</dl>
					<dl>
						<dt>订单来源</dt>
					...
					<dl>
						...
						<dd>
							<!--用于调用查询物流脚本方法的按钮。-->
					   		<a onclick="doTrack('${transInfo.trackNo}')"><font color="blue">${transInfo.trackNo}</font></a>
					   	</dd>
					</dl>
					<dl>
						<dt>账单国家</dt>
					...
					<dl>
						<dt>银行真实交易金额</dt>
						<dd>
							<c:if test="${transInfo.bankRealAmount==0.0 || empty transInfo.bankRealAmount}">无法获取</c:if>
							<c:if test="${transInfo.bankRealAmount!=0.0 || ! empty transInfo.bankRealAmount }">${transInfo.bankRealCurrency} ${transInfo.bankRealAmount}</c:if>
						</dd>
					</dl>
					<dl>
						<dt>银行真实交易汇率</dt>
						<dd>
							<c:if test="${transInfo.bankRealAmount==0.0 || empty transInfo.bankRealAmount}">无法获取</c:if>
							<c:if test="${transInfo.bankRealAmount!=0.0 || ! empty transInfo.bankRealAmount}">${transInfo.bankRealRate}</c:if>
						</dd>
					</dl>
					<table width="100%" class="list">
						<tr>
							<td colspan="3">货物信息</td>
						</tr>
						<tr>
							<td>货物名称</td>
							<td>货物数量</td>
