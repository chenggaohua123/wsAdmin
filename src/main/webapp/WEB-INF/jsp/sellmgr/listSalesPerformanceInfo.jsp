...
<body>
<form id="pagerForm" method="post" action="<%=path %>/sellmgr/salesPerformanceInfoList">
	<input type="hidden" name="pageNum" value="${page.nowPage }" />
	<input type="hidden" name="numPerPage" value="${page.pageSize}" />
</form>
<div class="pageHeader">
	<form rel="pagerForm" onsubmit="return navTabSearch(this);" action="<%=path %>/sellmgr/salesPerformanceInfoList" method="post">
	<div class="searchBar">
    ...
    <div class="panelBar">
		<ul class="toolBar">
		<li><a class="add" href="<%=path %>/sellmgr/exportsalesPerformanceInfo" target="dwzExport" targetType="navTab"rel="addCurrency" width="550" height="300" mask="true"><span>导出明细</span></a></li>
		</ul>
	</div>
			...
			<tbody>
				<c:forEach items="${page.data}" var="info">
					<tr>
						...
						<td>
						<a href="<%=path %>/sellmgr/exportsalesPerformanceInfo?userName=${info.sellName}&year=${info.comYear}&month=${info.comMonth}&currency=${info.currency}" target="dwzExport" targetType="navTab"rel="querySalesPerformance" mask="true"><span>导出</span></a>
						</td>
					</tr>
				</c:forEach>
			</tbody>
