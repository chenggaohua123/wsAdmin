...
<script type="text/javascript">
	 	...
	 	function resplitMessage(str){
	 		...
	 		if(spliIndex1==0){
	 			var flag = str.substring(spliIndex1, spliIndex2+1)
	 			if(flag=='{merNo}'){
	 				$('#content').append('<p><label>程序名：</label><select name="tableName"><option value="{merNo}" selected="selected">商户号</option><option value="{activeTime}">时间值</option><option value="{time}">笔数值</option></select><a href="javascript:void(0)" onclick="deleteElement(this)"><img alt="删除图片" src="'+path+'/dwz/uploadify/img/del.png"></a></p>');
	 			}
	 			if(flag=='{activeTime}'){
	 				$('#content').append('<p><label>程序名：</label><select name="tableName"><option value="{merNo}">商户号</option><option value="{activeTime}" selected="selected">时间值</option><option value="{time}">笔数值</option></select><a href="javascript:void(0)" onclick="deleteElement(this)"><img alt="删除图片" src="'+path+'/dwz/uploadify/img/del.png"></a></p>');
	 			}
	 			if(flag=='{time}'){
	 				$('#content').append('<p><label>程序名：</label><select name="tableName"><option value="{merNo}">商户号</option><option value="{activeTime}">时间值</option><option value="{time}" selected="selected">笔数值</option></select><a href="javascript:void(0)" onclick="deleteElement(this)"><img alt="删除图片" src="'+path+'/dwz/uploadify/img/del.png"></a></p>');
	 			}
	 			if(flag=='{respMsg}'){
	 				$('#content').append('<p><label>程序名：</label><select name="tableName"><option value="{merNo}">商户号</option><option value="{activeTime}">时间值</option><option value="{time}" selected="selected">笔数值</option><option value="{respMsg}" selected="selected">失败原因</option></select><a href="javascript:void(0)" onclick="deleteElement(this)"><img alt="删除图片" src="'+path+'/dwz/uploadify/img/del.png"></a></p>');
	 			}
