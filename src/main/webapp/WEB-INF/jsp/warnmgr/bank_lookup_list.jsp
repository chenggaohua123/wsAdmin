...
<body>
<form id="pagerForm" method="post" action="warnmgr/queryBankList">
...
<div class="pageHeader">
	<form onsubmit="return dialogSearch(this);" action="warnmgr/queryBankList" method="post" rel="pagerForm" class="pageForm required-validate">
		<div class="searchBar">
			<table class="searchContent">
				<tr>
						<td>银行名称：</td>
						<td><input type="text" name="bankName"
							value="${param.bankName}" /></td>
					</tr>
			</table>
			<div class="subBar">
				<ul>
...
<div class="pageContent">
	...
	<table class="table" width="100%" layoutH="120">
		...
		<tbody>
			<c:forEach items="${requestScope.result.data}" var="bank"
				varStatus="var">
				<tr target="bankId" rel="${bank.id}">
					...
					<td>${bank.port}</td>
					<td>
						<c:if test="${bank.isProxy==0}">不使用</c:if>
						<c:if test="${bank.isProxy==1}">使用代理连接</c:if>
					</td>
