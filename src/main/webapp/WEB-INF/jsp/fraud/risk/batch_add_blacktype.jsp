...
<script type="text/javascript">
        ...
        {	
            $("#selectFile").uploadify({
                ...
                'uploader': '<%=request.getContextPath() %>/manager/fraud/risk/batchAddBlackTextInfo?jsessionid=<%=request.getSession().getId()%>',
                ...
                'onUploadSuccess':function(file,data,response){
                	...
                	for(var i = 0;i<list.length;i++){
                		table+="<tr><td>"+list[i].blackType+"</td><td>"+list[i].blackText+"</td><td>"+list[i].createBy+"</td></tr>";
                	}
                	table+="</tbody></table>";
...
<div class="pageHeader">
		...
		<div id="fileQueue" class="fileQueue">
			...
			<a href="<%=request.getContextPath()%>/template/batchaddBlack.txt" target="_blank">模板下载</a>
		</div>
</div>
