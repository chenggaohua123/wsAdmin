...
<body>
<form id="pagerForm" method="post" action="fraud/rule/rules_list">
...
<div class="pageHeader">
	<form onsubmit="return navTabSearch(this);" action="fraud/rule/rules_list" method="post" rel="pagerForm" class="pageForm required-validate">
	<div class="searchBar">
...
<div class="pageContent">
	<div class="panelBar">
		<ul class="toolBar">
			<li><a class="add" href="<%=path %>/fraud/rule/goAddRules" target="dialog" target="dialog" rel="addrule"  mask="true" width="600" ><span>添加</span></a></li>
			<li><a class="edit" href="<%=path %>/fraud/rule/goUpdateRules?id={ruleId}" target="dialog" rel="updateRule" width="500" height="370" mask="true" ><span>修改</span></a></li>
		</ul>
	</div>
	<div id="w_list_print">
		...
		<tbody>
			<c:forEach items="${requestScope.result.data}" var="r"
				...
				<tr target="ruleId" rel="${r.ruleId}">
					...
					<td>${r.ruleName}</td>
					<td>
						<a href="fraud/rule/params_Value?paramId=${r.paramId}" target="dialog" mask="true" target="dialog" style="color: blue;" minable="false" atl="查看参数的值" width="500" height="400">${r.paramDescName}</a>
					</td>
					<td>
						<a href="fraud/rule/params_Value?paramId=${r.ruleParamValueId}" target="dialog" mask="true" target="dialog" style="color: blue;" minable="false" atl="查看参数的值" width="500" height="400">${r.ruleParamValueDescName}</a>
					</td>
					<td>
						<a href="fraud/rule/queryProcessClass?processClassId=${r.processClassId}" target="dialog" mask="true" target="dialog" style="color: blue;" minable="false" atl="查看参数的值" width="500" height="400">${r.processClassName}</a>
					</td>
