...
function batchAcceptCheckUpdateById(){
	if(clickFlag){
		...
		if(!str){
			...
			var url="<%=path %>"+"/whiteListMagage/addWhiteRiskInfos";
			$.ajax({
				url:url,
				type:'post',
				dataType:'json',
		...
		alert("您已经添加了一次,请重新上传文件添加");
	}
}
</script>
<body>

<div id="div_data2" style="width:100%;height:100%;clear:#183152;" >
	<div ><br/><h3 style="color:red;">错误信息</h3></div>
	<div id="showData2">
			${errorInfo }
	</div>
</div>
<div style="width:100%;height:100%;clear:#183152;text-align: left;">
		<a class="edit" href="javaScript:batchAcceptCheckUpdateById()"><span style="color:green;">批量添加</span></a>
			<a class="edit" href="javaScript:showData2()"><span style="color:green;">显示错误信息</span></a>
		<span style="color:red;">警告：点击批量添加后,数据将添加</span>
	...
	<table class="list" width="100%" targetType="navTab" layoutH="140" style="text-align: center;">
		...
		<tbody>
			<c:forEach items="${whiteList}" var="white">
				<tr  target="transCheckID" rel="${white.id }"   >
					<td>
						<input type="hidden" id="ids${white.id }"/>
						<input type="checkbox" name="selectAcceptAll_ids" value="${white.id },${white.merNo},${white.terNo},${white.blackType},${white.type},${white.blackText},${white.enableFlag},${white.remark}">
					</td>
					<td>${white.merNo}</td>
					<td>${white.terNo}</td>
					<td>${white.blackType}</td>
					<td>${white.type=='1'?'<font color=red>只接受</font>':'例外'}</td>
					<td>${white.blackText}</td>
					<td>${white.remark}</td>
					<td>${funcs:getStringColumnKey('RATESTATUS',white.enableFlag,'未知状态')}</td>
				</tr>
			</c:forEach>
		</tbody>
	</table>
