...
function batchAcceptCheckUpdateById(){
	...
	if(!str){
		...
		var url="<%=path %>"+"/refund/batchBankRefund";
		$.ajax({
			url:url,
			type:'post',
			dataType:'json',
...
<div style="width:100%;height:100%;clear:#183152;text-align: left;">
		<a class="edit" href="javaScript:batchAcceptCheckUpdateById()"><span style="color:green;">批量退款</span></a>
			<a class="edit" href="javaScript:showData2()"><span style="color:green;">显示错误信息</span></a>
		<span style="color:red;">警告：点击批量退款后，退款会直接提交到银行</span>
	...
	<table class="list" width="100%" targetType="navTab" layoutH="140" style="text-align: center;">
		...
		<tbody>
			<c:forEach items="${bankRefundList}" var="trans">
				<tr  target="transCheckID" rel="${trans.tradeNo }"   >
					...
					<td>${trans.tradeNo }</td>
					<td>${trans.bankAmount }</td>
					<td>${trans.refundAmount }</td>
				</tr>
			</c:forEach>
		</tbody>
	</table>
