...
function showData(){
	$("#div_data").toggle();
	var data = '${batchCardBinInfolistsb}';
	var arr = data.split(';');
	var str = '<br/>';
...
function batchAcceptCheckUpdateById(){
	...
	if(!str){
		...
		var url="<%=path %>"+"/sysmgr/batchUpdateCardBinInfo";
		$.ajax({
			url:url,
			type:'post',
			dataType:'json',
...
<div class="pageContent" style="width:100%;height:100%;clear:#183152;">
	<div class="panelBar">
		<ul class="toolBar">
			<li><a class="edit" href="javaScript:batchAcceptCheckUpdateById()"><span style="color:green;">批量修改</span></a></li>
			<li class="line">line</li>
			<li><a class="edit" href="javaScript:showData()"><span style="color:green;">显示未匹配数据</span></a></li>
			<li><span style="color:red;">警告：点击批量修改，系统数据会直接修改</span></li>
		</ul>
	</div>
	...
	<table class="list" width="100%" targetType="navTab" layoutH="140" style="text-align: center;">
		...
		<tbody>
			<c:forEach items="${list}" var="trans">
				<tr  target="transCheckID" rel="${trans.bin }"   >
					<td>
						<input type="hidden" id="ids${trans.bin}">
						<input type="checkbox" name="selectAcceptAll_ids" value="${trans.bin}">
					</td>
					<td>${trans.bin }</td>
					<td>${trans.brand }</td>
