...
(function($){
	...
	var loadOption=function(combox,url){
		$.ajax({
			type:'POST', 
			dataType:"json", 
			url:url, 
			...
			success: function(json){
				if (!json) return;
				var html = '';
				var relValue = combox.attr("relValue");
				var relText = combox.attr("relText");
				var selectedValue = combox.attr("selectedValue")==undefined?"":combox.attr("selectedValue");
				for(var i = 0;i<json.length;i++){
					var selected = "";
					if(selectedValue == json[i][relValue]){
						selected ="selected";
					}
					html += '<option  value="'+json[i][relValue]+'" '+selected+'>' + json[i][relText] + '</option>';
				}
				combox.append(html);
			},
			error: DWZ.ajaxError
